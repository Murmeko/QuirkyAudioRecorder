...
class AudioController: NSObject, StereoLevelsProvider {
  ...
  func selectRecordingOption(_ option: RecordingOption, orientation: Orientation, completion: (StereoLayout) -> Void) {
    ...
    guard let preferredInput = session.preferredInput,
          ...
          let newDataSource = dataSources.first(where: { $0.orientation?.rawValue == option.dataSourceName }),
    ...
    completion(StereoLayout(orientation: newDataSource.orientation!, stereoOrientation: session.inputOrientation))
  }
